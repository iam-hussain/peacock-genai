...
/**
 * Cache for base-info.json to avoid repeated disk reads
 */
let baseInfoCache: { data: BaseInfo; mtimeMs: number } | null = null;

/**
 * Load base info data from JSON file (with mtime-based cache)
 */
async function loadBaseInfo(): Promise<BaseInfo | null> {
  try {
    ...
    // Return cached data if file has not changed
    if (baseInfoCache && baseInfoCache.mtimeMs === stat.mtimeMs) {
      return baseInfoCache.data;
    }

    const fileContent = await fs.readFile(filePath, "utf-8");
    const parsed = JSON.parse(fileContent) as BaseInfo;

    baseInfoCache = { data: parsed, mtimeMs: stat.mtimeMs };
    return parsed;
  } catch (error) {
    logger.warn(
      "Failed to load base-info.json:",
...
function generateMemberSummary(members: BaseInfo["member"]): string {
  ...
  activeMembers.forEach((member) => {
    const name = [member.firstName, member.lastName]
      ...
      .join(" ")
      .trim();
    summary += `- ${name} (${member.username}) - ${member.status}${member.accessLevel === "WRITE" ? ", Write Access" : ""}\n`;
  });
  ...
  if (activeVendors.length > 0) {
    ...
    activeVendors.forEach((vendor) => {
      const name = [vendor.firstName, vendor.lastName]
        ...
        .join(" ")
        .trim();
      const startDate = vendor.startedAt
        ? new Date(vendor.startedAt).toLocaleDateString()
        : "N/A";
      const endDate = vendor.endedAt
        ? new Date(vendor.endedAt).toLocaleDateString()
        : "Ongoing";
