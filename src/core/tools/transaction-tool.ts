...
import { getPeacockApiUrl, getSessionCookie } from '@/lib/auth'
      ...
      func: async ({
        ...
        try {
          ...
          const response = await fetch(
            `${baseUrl}/api/transaction?${params.toString()}`,
            {
              method: 'POST',
              headers: {
    ...
    new DynamicStructuredTool({
      ...
      func: async (payload) => {
        try {
          ...
          const response = await fetch(`${baseUrl}/api/transaction/create`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              Cookie: cookie,
